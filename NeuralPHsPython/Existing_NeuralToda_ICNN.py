...
class ReOrPHNN(nn.Module):
    ...
    def __init__(self, H_net, V):
        ...
        self.gamma = torch.nn.Parameter(torch.tensor(1., requires_grad=True))
    ...
    def forward(self, x, u):
        ...
        H_x  = self.H_net(x)
        dHdx = autograd.grad(H_x, x, create_graph=True)[0]
...
class FICNN(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.fc_z1 = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.fc_z1.weight = nn.Parameter(torch.nn.functional.softplus(torch.randn_like(self.fc_z1.weight)))

        # Hidden to second hidden layer
        self.fc_x2 = nn.Linear(input_dim, hidden_dim)
        self.fc_z2 = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.fc_z2.weight = nn.Parameter(torch.nn.functional.softplus(torch.randn_like(self.fc_z2.weight)))

        # Input to second hidden layer
        self.fc_x3 = nn.Linear(input_dim, hidden_dim)
    ...
    def forward(self, x):
        z1 = self.activation(self.fc_x1(x))
        z2 = self.activation(self.fc_x2(x) + torch.nn.functional.softplus(self.fc_z1.weight) @ z1.T)
        z3 = self.activation(self.fc_x3(x) + torch.nn.functional.softplus(self.fc_z2.weight) @ z2.T)
        return self.fc_out(z3.T)
...
# Define H_net using FICNN
nncells = 20
H_net = FICNN(nx, nncells)
...
for epoch in range(1, num_epochs + 1):
    ...
    for x_batch, u_batch, x_dot_batch in train_loader:
        ...
        zero_input = torch.zeros_like(x_batch[0]).unsqueeze(0)  # Shape (1, nx)
        h_zero = mor_PH.H_net(zero_input)  # Compute H_net(0)
        constraint_loss = 1e-1 * torch.norm(h_zero, p=2)  # Penalize nonzero values

        # Compute loss
        loss = criterion(x_dot_pred, x_dot_batch)

        # Constraint loss
        total_loss = loss + constraint_loss # Combine with main loss
        total_loss.backward()
...
## Convexity Check for FICNN
if isinstance(H_net, nn.Module) and hasattr(H_net, 'fc_z1') and hasattr(H_net, 'fc_z2'):
    with torch.no_grad():
        z1_weights = torch.nn.functional.softplus(H_net.fc_z1.weight)
        z2_weights = torch.nn.functional.softplus(H_net.fc_z2.weight)
        is_z1_positive = torch.all(z1_weights >= 0).item()
        is_z2_positive = torch.all(z2_weights >= 0).item()
        print(f"\nConvexity check:")
        print(f" - All entries in softplus(fc_z1.weight) >= 0: {is_z1_positive}")
        print(f" - All entries in softplus(fc_z2.weight) >= 0: {is_z2_positive}")
else:
    print("\nH_net is not FICNN, skipping convexity check.")
...
plt.figure(figsize=(8, 5))
plt.plot(sq_err,  label="Squared Error of full-order state",    color="blue")
plt.legend()
...
plt.figure(figsize=(8, 5))
plt.plot(test_splt[:len(y)], y,       label = "Est. output", color="blue")
plt.plot(test_splt[:len(y)], test_DY[:len(y)], label = "Actual output", color="red")
plt.legend()


plt.figure(figsize=(8, 5))
plt.plot(test_splt[:len(H)], H, label = "Est. Hamiltonian", color="blue")
